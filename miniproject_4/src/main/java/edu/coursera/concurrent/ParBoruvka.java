...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
    	...
    	while ((n = nodesLoaded.poll()) != null) {
    		if (!n.lock.tryLock()) {
    			continue; // another thread is working on node n
    		}
    		...
    		final Edge<ParComponent> e = n.getMinEdge(); // retrieve n's edge with minimum length
    		if (e == null) {
    			n.lock.unlock();
    		...
    		// merge other node into node n
    		n.merge(other, e.weight());
    		n.lock.unlock();
    		other.lock.unlock();
    		
    		// add newly merged n back in the nodesLoaded queue
    		nodesLoaded.add(n);
    	}
    }
